...
HEADER;
	...
	file_put_contents(dirname(__DIR__) . '/src/lang/CustomKnownTranslationKeys.php', ob_get_clean());

	echo "Done generating CustomKnownTranslationKeys.\n";
}
...
HEADER;
	...
	foreach(Utils::stringifyKeys($languageDefinitions) as $key => $value){
		...
		echo "\t\treturn new $translationContainerClass(CustomKnownTranslationKeys::" . constantify($key) . ", [";
	...
	file_put_contents(dirname(__DIR__) . '/src/lang/CustomKnownTranslationFactory.php', ob_get_clean());

	echo "Done generating CustomKnownTranslationFactory.\n";
}

$lang = parse_ini_file(Path::join($argv[1], 'resources', 'lang', 'data', "eng.ini"), false, INI_SCANNER_RAW);
if($lang === false){
